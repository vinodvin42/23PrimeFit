...
export class MarketplaceService {
  ...
  async enroll(user: AuthUser, listingId: string) {
    const listing = await this.prisma.marketplaceListing.findFirst({
      where: { id: listingId, published: true, tenant: { active: true } },
    });
    if (!listing) throw new ForbiddenException('Listing is unavailable');
    const [enrollment] = await this.prisma.$transaction([
      this.prisma.marketplaceEnrollment.upsert({
        where: {
          tenantId_userId: { tenantId: listing.tenantId, userId: user.id },
        },
        update: {},
        create: { tenantId: listing.tenantId, userId: user.id, listingId },
      }),
      this.prisma.clientMembership.upsert({
        where: {
          tenantId_userId: { tenantId: listing.tenantId, userId: user.id },
        },
        update: { status: ClientMembershipStatus.ACTIVE, joinedAt: new Date() },
        create: {
...
export class MarketplaceController {
  ...
  @Post('listing') publish(
    ...
    return this.service.publish(user, body, tenant);
  }
  @Post('enroll') enroll(
    @CurrentUser() user: AuthUser,
    @Body() body: { listingId: string },
  ) {
    return this.service.enroll(user, body.listingId);
  }
}
...
@Module({
  ...
  controllers: [MarketplacePublicController, MarketplaceController],
})
