...
export class PartnerService {
  ...
  async authenticate(key?: string) {
    ...
    const apiKey = await this.prisma.apiKey.findFirst({
      where: { secretHash: hash(key), revokedAt: null },
    });
  ...
  async usage(key?: string) {
    const apiKey = await this.authenticate(key);
    return this.prisma.usageMeter.findMany({
...
export class PartnerAdminController {
  ...
  @Post('keys') key(
    ...
    return this.service.createKey(user, body.name, tenant);
  }
  @Post('webhooks') webhook(
    ...
    return this.service.registerWebhook(user, body, tenant);
  }
}
@ApiTags('partner')
...
export class PartnerApiController {
  constructor(private readonly service: PartnerService) {}
  @Get('clients') clients(@Headers('x-api-key') key?: string) {
    return this.service.clients(key);
  }
  @Get('usage') usage(@Headers('x-api-key') key?: string) {
    return this.service.usage(key);
  }
}
...
@Module({
  ...
  controllers: [PartnerAdminController, PartnerApiController],
})
