...
export class VisionService {
  ...
  async posture(
    user: AuthUser,
    body: { tenantId?: string; imageUrl?: string; notes?: string },
  ) {
    const summary =
      'Draft posture screen: review hip and shoulder alignment with a qualified coach.';
    const analysis = await this.prisma.postureAnalysis.create({
      data: {
        ...
        findingsJson: JSON.stringify([
          { area: 'alignment', note: body.notes ?? 'Visual check required' },
        ]),
      },
    });
    await this.prisma.aiInsight.create({
      data: {
        ...
        userId: user.id,
        category: 'vision',
        title: 'Posture analysis awaiting review',
        body: summary,
        severity: 'info',
        source: 'vision-rules',
  ...
  mine(user: AuthUser) {
    return Promise.all([
      this.prisma.foodVisionLog.findMany({
        where: { userId: user.id },
        orderBy: { createdAt: 'desc' },
        take: 30,
      }),
      this.prisma.postureAnalysis.findMany({
        where: {
          userId: user.id,
          ...(user.role === 'CLIENT'
  ...
  async approve(
    coach: AuthUser,
    id: string,
    body: { status: 'APPROVED' | 'REJECTED'; coachNotes?: string },
  ) {
    if (!['COACH', 'ADMIN'].includes(coach.role))
      throw new ForbiddenException('Coach role required');
    return this.prisma.postureAnalysis.update({
      where: { id },
      data: {
        status: body.status,
...
export class VisionController {
  ...
  @Post('food') food(
    ...
    @Body() body: Parameters<VisionService['food']>[1],
  ) {
    return this.service.food(user, body);
  }
  @Post('posture') posture(
    @CurrentUser() user: AuthUser,
    @Body() body: Parameters<VisionService['posture']>[1],
  ) {
    return this.service.posture(user, body);
  }
  @Get('mine') mine(@CurrentUser() user: AuthUser) {
    return this.service.mine(user);
  }
  @Patch('posture/:id/review') review(
    @CurrentUser() user: AuthUser,
    @Param('id') id: string,
    @Body() body: { status: 'APPROVED' | 'REJECTED'; coachNotes?: string },
  ) {
